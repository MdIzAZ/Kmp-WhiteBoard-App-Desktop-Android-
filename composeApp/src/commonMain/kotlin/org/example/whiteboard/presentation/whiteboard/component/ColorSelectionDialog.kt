...
fun ColorSelectionDialog(
    ...
    var hexInput by remember { mutableStateOf(controller.selectedColor.value.toHexString()) }

    var isWrongHexInput by remember { mutableStateOf(false) }

    fun handleHexInputChange(input: String) {
        if (input.length == 6 && input.all { it.isHexDigit() }) {
            isWrongHexInput = false
            val color = Color("ff$input".toLong(16))
            controller.selectByColor(color = color, fromUser = true)
        } else {
            isWrongHexInput = true
        }
                ...
                Column(
                    ...
                    HsvColorPicker(
                        ...
                        onColorChanged = { colorEnvelope ->
                            hexInput = colorEnvelope.color.toHexString()
                        }
                    )
                    ...
                    OutlinedTextField(
                        ...
                        onValueChange = {
                            hexInput = it
                            handleHexInputChange(it)
                        },
                        label = { Text(text = "Enter Hex Color Code") },
                        prefix = { Text(text = "#") },
                        singleLine = true,
                        isError = isWrongHexInput,
                        supportingText = { Text(text = if (isWrongHexInput) "Enter a valid Hex Code" else "") }
                    )
                }
